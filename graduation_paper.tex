...
\begin{document}
  ...
  \chapter{序論}
    ...
    \section{本論文の構成}
      第$2$章で以降に登場する数学的概念を定義し、第$3$章で$2$次元チェバをどのように一般化して\mathlib 4に追加するかを議論する。
  ...
  \chapter{型理論と証明支援系}
    ...
    理論とは言語・公理・推論規則の3つ組で一意に決定されるものであり、型理論もまた同様にこの3つ組の種類によって数多くの種類が存在する。

    また、ZFとZFCの違いのような大きな違いではなく、さらに細かい観点の違いで異なる理論となるため、名前が付けられていない理論も数多く存在する。
    
    さらに、型理論では型そのものもまた型を持つという階層（宇宙）を備えることが多い。これにより「全ての型の型」のような自己言及的構成によるパラドックスを避けながら、抽象的な数学構造を安全に扱える。Leanではこの階層を \texttt{Sort} のレベル（\texttt{Prop}, \texttt{Type} など）として管理し、命題（証明の世界）とデータ（計算の世界）の取り扱いを区別する設計を採用している。
    
    本研究で扱うチェバの定理の形式化では、アフィン空間・加群・線形写像といった代数的構造を型として表現し、それらの公理や補題を型理論上の命題として表す。これにより、自然言語の推論にありがちな飛躍や見落としを、型付けと証明検査によって局所的に遮断できる。

    \section{証明支援系の概要}
    証明支援系とは、数学の定義・定理・証明を形式言語で記述し、機械的に検証するためのソフトウェア。
    
    証明支援系の目的は「証明を自動で発見する」ことよりも、「与えられた証明が正しいことを厳密に検査する」ことにある。
    
    これにより、長大な証明や複雑な依存関係を持つ理論でも、最終的な正当性を信頼できる形で担保できる。

    Leanは依存型理論に基づく証明支援系であり、プログラムと証明を同一言語で扱える。ユーザーは定義や補題を積み上げ、最終的に定理を証明する。証明の書き方には大きく二系統がある。ひとつは項として証明を直接構成する「項モード」、もうひとつは人間の推論手順に近い形で証明状態を変形していくtacticモードである。いずれの場合も、最終的に得られた項が型検査を通過すれば証明は受理される。
    
    \mathlib はLean上の巨大な数学ライブラリであり、代数・解析・幾何などの定義と定理が体系的に蓄積されている。本研究の方針は、（1）既存の\mathlib の抽象化（アフィン空間、加群、アフィン独立、アフィン結合など）を最大限再利用し、（2）不足する補題を局所的に追加することで、チェバの定理の高次元一般化とその逆を「\mathlib の流儀」に沿って形式化する、というものである。
    これにより、単発の形式化に留まらず、今後の幾何・線形代数の定理形式化へ再利用可能な部品として寄与することを狙う。

    \section{型理論の簡単な概念}

      \subsection{型}
        \begin{definition}[型]
          型（type）とは、項（term）が属する"分類"である。Leanでは「$t:T$」と書いて、項 $t$ が型 $T$ を持つ（= $T$ の要素である）ことを表す。
          また、命題も型として表現される。命題 $P$ が成り立つことは「$P$ という型に項（証明）が存在する」こととして表される。

          （補足）Leanには \texttt{Prop} と \texttt{Type} 系の区別があり、前者は証明（論理）側、後者はデータ（計算）側として主に用いられる。ただしどちらも "型" である点は同じで、違いは計算規則や情報消去に関わる設計上の性質にある。
        \end{definition}

      \subsection{型クラス}
        \begin{definition}[型クラス]
          型クラスとは、「ある型に追加構造（演算や公理）を与えるための型」であり、Leanでは型クラスとして宣言された型の項を インスタンス と呼ぶ。
          典型例として、\texttt{Group $G$} は「型 $G$ が群構造を持つ」ことを表す型クラスであり、そのインスタンスは単位元・積・逆元と群公理のデータを含む。

          型クラスの実用上の核心は 型クラス推論（typeclass resolution） にある。証明や定義の途中で群演算が必要になったとき、ユーザーが毎回「この群構造を使う」と手で渡さなくても、Leanがコンテキストから適切なインスタンスを自動探索して補完してくれる。

          \mathlib が巨大化しても比較的一貫した書き味を保てるのは、この仕組みによるところが大きい。
        \end{definition}

      \subsection{インスタンス}
        \begin{definition}[インスタンス]
          インスタンスとは、型クラスの具体的な項である。たとえば「整数 $Z$ は加法群である」という事実は、Leanでは「\texttt{AddGroup $Z$} のインスタンスが存在する」として表現される。
          同様に、本研究で用いる「$V$ は $k$-加群」「$P$ は $V$ 上のアフィン空間」といった構造は、対応する型クラスのインスタンスとして与えられることで、以後の定理記述・証明で自動的に利用できる。
        \end{definition}
  ...
  \section{一般化の設計空間}\section{証明の道具}
  \subsection{アフィン空間}
  ...
  \subsection{共点性}
    ...
    \begin{definition}[（広義の）単体 / アフィン単体]
      ...
      これらの点のアフィン包（affine hull）を（広義の）$n$-単体（あるいは \emph{アフィン} $n$-単体）と呼ぶ．
      すなわち，
      \[
  ...
  \subsection{チェビアンの足}
    ...
    \section{\mathlib 4に加える一般化の選定}
      ...
      \subsection{候補の比較}
        \subsubsection{候補A：周囲空間のみ高次元化}
          定理の主張が変わらないため、一般化としての価値が低い。

        \subsubsection{候補B：$n$単体版,solopede,1-チェビアン}
          定理の主張が変わらないため、一般化としての価値が低い。

        \subsubsection{候補C：$n$単体版,solopede,2-チェビアン}
          定理の主張が変わらないため、一般化としての価値が低い。

        \subsubsection{候補D：$n$単体版,multipede}
          定理の主張が変わらないため、一般化としての価値が低い。
  ...
  \chapter{Leanでの実装}
    ...
    \section{設計方針}
      \subsection{設計の問題点}
        後述するがチェバの定理を一般化すると順方向と逆方向で必要な仮定が異なる。
        そのため、順方向と逆方向で別々のモジュールを追加する必要がある。
        ただし必要十分性を主張する2次元のチェバの定理の一般化として
